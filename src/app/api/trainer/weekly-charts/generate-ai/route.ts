// src/app/api/trainer/weekly-charts/generate-ai/route.ts
...
// POST: Generate chart using AI (Trainer)
export async function POST(request: NextRequest) {
    try {
        ...
        // Verify that this trainer is assigned to this membership
        ...
        if (membershipError || !membership) {
            return NextResponse.json(
                { error: 'You are not assigned as trainer for this membership or membership is not active' },
                { status: 403 }
            );
        }
        ...
        // Check if trainer can upload based on chart responsibility
        if (!chartResp.canTrainerUpload || chartResp.shouldUpload !== 'trainer') {
            return NextResponse.json(
                { error: `You cannot upload charts for this membership. ${chartResp.reason}` },
                { status: 403 }
            );
        }
...
Make it practical, safe, and tailored to their specific needs, goals, and dietary restrictions.`;
        ...
        const { data: chart, error: chartError } = await supabaseAdmin
            ...
            .insert({
                ...
                file_url: null,
                created_by: trainer.id // Set trainer ID for trainer-created charts
            })
            .select()
            .single();
        ...
        if (membership.user_id) {
            await supabaseAdmin
                ...
                .insert({
                    ...
                    content: `Your trainer has added a new ${chart_type} chart for Week ${week_number} to your membership.`,
                    is_read: false
                });
        }
