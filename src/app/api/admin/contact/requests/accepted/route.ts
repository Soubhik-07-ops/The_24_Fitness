    ...
    try {
        ...
        try {
            ...
            if (userIds.length > 0) {
                ...
                if (profs) profiles = Object.fromEntries(profs.map((p: any) => [p.id, { full_name: p.full_name, email: p.email }]));
            }
        ...
        console.error('Admin accepted contact requests error:', err);
        return NextResponse.json({ error: err.message || 'Failed to fetch accepted requests' }, { status: 500 });
    }
}
